...
class ShadowNet(cnn_basenet.CNNBaseModel):
    ...
    def _feature_sequence_extraction(self, inputdata, name):
        ...
        with tf.variable_scope(name_or_scope=name):
            ...
            relu7 = self.relu(inputdata=bn7, name="relu7")

            # Added for NRC images.

            max_pool7 = self.maxpooling(
                inputdata=relu7, kernel_size=[2, 1], stride=[2, 1], name="max_pool7"
            )
            ...
            bn8 = self.layerbn(
                inputdata=conv8, is_training=self._is_training, name="bn8"
            )
    ...
    def _sequence_label(self, inputdata, name):
        ...
        with tf.variable_scope(name_or_scope=name):
            ...
            logits = tf.reshape(
                logits, [shape[0], shape[1], self._num_classes], name="logits_reshape"
            )
            ...
            rnn_out = tf.transpose(
                logits, [1, 0, 2], name="transpose_time_major"
    ...
    def compute_loss(self, inputdata, labels, name, reuse):
        ...
        inference_ret = self.inference(inputdata=inputdata, name=name, reuse=reuse)
