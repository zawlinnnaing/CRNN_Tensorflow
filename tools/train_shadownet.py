...
def init_args():
    ...
    parser.add_argument(
        ...
        "--model_path",
        type=str,
        default="model/crnn_syn90",
        help="Model path for saving checkpoint.",
    )
...
def compute_net_gradients(
    images, labels, net, optimizer=None, is_net_first_initialized=False
):
    """
...
def train_shadownet(
    ...
    train_inference_ret, train_ctc_loss = shadownet.compute_loss(
        inputdata=train_images, labels=train_labels, name="shadow_net", reuse=False
    )
    val_inference_ret, val_ctc_loss = shadownet_val.compute_loss(
        inputdata=val_images, labels=val_labels, name="shadow_net", reuse=True
    )
    ...
    with sess.as_default():
        ...
        while epoch < train_epochs:
            ...
            if epoch > 1 and CFG.TRAIN.EARLY_STOPPING:
                ...
                if (
                    cost_history[-1 - patience_counter] - cost_history[-1]
                    > CFG.TRAIN.PATIENCE_DELTA
                ):
                    patience_counter = 1
                else:
                    patience_counter += 1
                if patience_counter > CFG.TRAIN.PATIENCE_EPOCHS:
                    logger.info(
                        "Cost didn't improve beyond {:f} for {:d} epochs, stopping early.".format(
                            CFG.TRAIN.PATIENCE_DELTA, patience_counter
                        )
                    )
                    break
            ...
            if need_decode and epoch % 500 == 0:
                ...
                (
                    _,
                    train_ctc_loss_value,
                    train_seq_dist_value,
                    train_predictions,
                    train_labels_sparse,
                    merge_summary_value,
                ) = sess.run(
                ...
                avg_train_accuracy = evaluation_tools.compute_accuracy(
                    train_labels_str, train_predictions
                )

                if epoch % CFG.TRAIN.DISPLAY_STEP == 0:
                    logger.info(
                        "Epoch_Train: {:d} cost= {:9f} seq distance= {:9f} train accuracy= {:9f}".format(
                            epoch + 1,
                            train_ctc_loss_value,
                            train_seq_dist_value,
                            avg_train_accuracy,
                ...
                (
                    ...
                    val_predictions,
                    val_labels_sparse,
                ) = sess.run([val_ctc_loss, val_sequence_dist, val_decoded, val_labels])
                ...
                avg_val_accuracy = evaluation_tools.compute_accuracy(
                    val_labels_str, val_predictions
                )

                if epoch % CFG.TRAIN.DISPLAY_STEP == 0:
                    logger.info(
                        "Epoch_Val: {:d} cost= {:9f} seq distance= {:9f} train accuracy= {:9f}".format(
                            epoch + 1,
                            val_ctc_loss_value,
                            val_seq_dist_value,
                            avg_val_accuracy,
            ...
            if epoch % CFG.TRAIN.EPOCHS_PER_CHECKPOINT == 0:
                logger.info(
                    "Saving checkpoint for epoch {} in {}".format(
...
def train_shadownet_multi_gpu(
    ...
    sess_config = tf.ConfigProto(
        device_count={"GPU": CFG.TRAIN.GPU_NUM}, allow_soft_placement=True
    )
    sess_config.gpu_options.per_process_gpu_memory_fraction = (
        CFG.TRAIN.GPU_MEMORY_FRACTION
    )
    ...
    with sess.as_default():
        ...
        if weights_path is None:
            ...
            logger.info(
                "Restore model from last model checkpoint {:s}".format(weights_path)
            )
            saver.restore(sess=sess, save_path=weights_path)
            epoch = sess.run(tf.train.get_global_step())
        ...
        while epoch < train_epochs:
            ...
            _, train_loss_value, train_summary, lr = sess.run(
                fetches=[
                    train_op,
                    avg_train_loss,
            ...
            if epoch % CFG.TRAIN.DISPLAY_STEP == 0:
                logger.info(
                    "Epoch_Train: {:d} total_loss= {:6f} "
                    "lr= {:6f} mean_cost_time= {:5f}s ".format(
                        epoch + 1, train_loss_value, lr, np.mean(train_cost_time_mean)
                    )
                )
            ...
            if epoch % CFG.TRAIN.VAL_DISPLAY_STEP == 0:
                logger.info(
                    "Epoch_Val: {:d} total_loss= {:6f} "
                    " mean_cost_time= {:5f}s ".format(
                        epoch + 1, val_loss_value, np.mean(val_cost_time_mean)
                    )
                )
